...
def swamp():
    ...
    while True:
        try:
            choice_sondre=input(f"Slår {spiller.name} leir?(y/n)\n")
            choice_sondre=choice_sondre.lower()
            if choice_sondre=="y":
                print(f"{spiller.name} fikk samlet ved, men den var våt og det tok ikke fyr. {spiller.name} var for kald og sliten til å fortsette. {spiller.name} døde")
            elif choice_sondre=="n":
                print(f"Selv om {spiller.name} var kald, så forsatte {spiller.name} å gå. {spiller.name} møtte på en bil i skogen, som kjørte deg til T-banen. {spiller.name} kom hjem og koste deg i høstferien")
            else:
...
def path():
    """ Dette er sti-valget """
    print(f"{spiller.name} går på stien i 5km og kommer til en seter. Vil {spiller.name} ta deg en bolle?")
    while True:
        try:
            choice_sondre=input(f"Vil {spiller.name} ta deg en bolle?(y/n)\n")
            choice_sondre=choice_sondre.lower()
            if choice_sondre=="y":
                print(f"Bollen er veldig dyr, og {spiller.name} har ikke nok penger. {spiller.name} kan vaske eller løpe.")
                choice_sondre=input(f"Blir {spiller.name} igjen og vasker?(y/n)\n")
                choice_sondre=choice_sondre.lower()
                if choice_sondre=="y":
                    print(f"{spiller.name} ble der i fem timer, men {spiller.name} kom deg hjem")
                elif choice_sondre=="n":
                    print(f"{spiller.name} løp. En ansatte fanget deg. Politiet kom å hentet deg, og {spiller.name} havnet i fengsel. {spiller.name} koste deg på Bastøy og ble der resten av livet.")
                else:
                    print("Svaret er ikke y eller n")
                    raise ValueError
            elif choice_sondre=="n":
                print(f"{spiller.name} gikk videre, men kom deg til T-banen sulten.")
            else:
                print("Svaret er ikke y eller n")
                raise ValueError
            break
